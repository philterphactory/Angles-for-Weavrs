...
service_url_dict = {
    ...
    'delete_slideshow' : 'http://www.slideshare.net/api/2/delete_slideshow',
    'user_tags' : 'http://www.slideshare.net/api/2/get_user_tags', # enrike
}


class PyslideshareException(Exception) :
    pass
...
class pyslideshare:
    ...
    def return_data(self, json):
        ...
        if json and hasattr(json, 'SlideShareServiceError'):
            raise PyslideshareException(json.SlideShareServiceError.Message)
...
##            return None
    ...
    ##############################
    def get_slideshow_by_user(self, username_for=None,offset=None,limit=None,get_unconverted=False):
        """
        ...
        return self.make_call('slideshow_by_user', username_for=username_for, offset=offset,
                              limit=limit,get_unconverted=get_unconverted)
        
    def get_slideshow(self, slideshow_id=None, slideshow_url=None):
        """
        ...
        if not slideshow_id:
            if not slideshow_url :
                print >> sys.stderr, 'slideshow_id or slideshow_url are needed for this call.'
                sys.exit(1)
            else :
                return self.make_call('get_slideshow', slideshow_url=slideshow_url)
    ...
    def get_user_tags(self, username=None, password=None):
        """
        Method to get user tags
        """
        if not username or not password :
            print >> sys.stderr, 'Required parameters missing.'
            sys.exit(1)
        return self.make_call('user_tags', username=username, password=password)
...
make_slideshow_private, generate_secret_url, allow_embeds, share_with_contacts
    ...
    def delete_slideshow(self, slideshow_id=None, username=None, password=None):
        """
